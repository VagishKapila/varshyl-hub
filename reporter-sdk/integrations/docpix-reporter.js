/**
 * Varshyl Hub Reporter — DocPix Studio Integration
 *
 * Drop this file into the DocPix Studio backend (e.g. backend/src/hub-reporter.js)
...
/**
 * Initialize the hub reporter with DocPix Studio-specific metrics collection.
...
function initHubReporter(pool) {
  ...
  return startReporter({
    ...
    collector: async () => {
      // ── User counts (from DocPix Studio schema: users table) ─
      ...
      // DocPix Studio is free — MRR is from signing flow payments only
      ...
      return {
        ...
        trial_users: 0, // DocPix has no trial tier yet
        pro_users: proUsers,
        churned_users: 0,
    ...
    onSuccess: (metrics) => {
      console.log(`[HubReporter] DocPix Studio → Hub: ${metrics.total_users} users, ${metrics.pdfs_generated_24h} PDFs today`);
    },
    onError: (err, phase) => {
